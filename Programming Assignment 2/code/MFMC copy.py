...
def Q_learning(num_episodes=100000, gamma=0.9, epsilon=1, decay_rate=0.999999):
    """
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            if state not in Q_table:
                Q_table[state] = {action: 0 for action in range(6)}  # Initialize Q-values for all actions

            if state not in eta_table:
                eta_table[state] = {action: 0 for action in range(6)}  # Initialize update counters for each action

            # Epsilon-greedy action selection
            if np.random.rand() < epsilon:
                action = np.random.randint(0, 6)  # Explore: take a random action
            else:
                action = max(Q_table[state], key=Q_table[state].get)  # Exploit: take the action with the max Q-value
            ...
            if next_state not in Q_table:
                Q_table[next_state] = {action: 0 for action in range(6)}

            if next_state not in eta_table:
                eta_table[next_state] = {action: 0 for action in range(6)}

            # Learning rate is based on the number of updates for the state-action pair
            eta = 1 / (1 + eta_table[state][action])

            # Q-learning update rule
            # Q_table[next_state].get returns Q values; max will return the best action
